...
// Batch 6: Casinos #29-32 (Final batch)
const casinos = [
  { id: 29, name: 'Amunra', domains: ['amunra.com', 'amunra.casino', 'amun-ra.com', 'amunracasino.com'] },
  { id: 30, name: 'Zotabet', domains: ['zotabet.com', 'zotabet.casino', 'zota-bet.com', 'zotabet.io'] },
  { id: 31, name: 'Slotlords', domains: ['slotlords.com', 'slot-lords.com', 'slotlords.casino', 'slotlords.io'] },
  { id: 32, name: 'Cashalot', domains: ['cashalot.bet', 'cashalot.com', 'cashalot.casino', 'cash-a-lot.com'] }
];
...
async function main() {
  console.log('\n🚀 BATCH 6 - Direct Playwright Extraction (FINAL)');
  console.log(`📦 Processing ${casinos.length} casinos (#29-32)`);
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log('📊 BATCH 6 SUMMARY (FINAL)');
  console.log(`${'='.repeat(60)}`);
  console.log(`✅ Succeeded: ${succeeded}/${results.length}`);
  console.log(`❌ Failed: ${failed}/${results.length}`);
  console.log(`⏱️  Time: ${elapsed}s`);
  console.log(`📈 Progress: ${27 + succeeded}/32 casinos (${((27 + succeeded) / 32 * 100).toFixed(1)}%)`);
  
  await fs.writeFile(
    'batch6-results.json',
    JSON.stringify({ batch: 6, results, succeeded, failed, elapsed }, null, 2)
  );
  
  console.log('\n✅ Results saved to batch6-results.json\n');
  
  if (27 + succeeded === 32) {
    console.log('🎉🎉🎉 ALL 32 CASINOS COMPLETE! 🎉🎉🎉\n');
  }
}
